...
/**
 * Parses the DID into scheme and method (prefix) component.
 *
 * @example
 ...
 *
 * @returns {{scheme: string, prefix: string}} Returns the scheme & method prefix.
 */
export function parseDid({did}) {
  ...
  const [scheme, prefix] = did.split(':');

  return {scheme, prefix};
}

/**
 * Takes in a did and determines if it has url characters in it.
 *
 * @param {oject} options - Options to use.
