...
class VeresOneDidDoc {
  ...
  generateId({keyPair, didType = 'nym', env = 'dev'}) {
    ...
    return this.createCryptonymDid({
      ...
      encoding: typeof keyPair.publicKey === 'string' ? 'ed25519' : 'forge',
      env
    });
  }
  ...
  /**
   * Creates a cryptonym DID from a public key with encoding `pem`, `ed25519`,
  ...
  createCryptonymDid({publicKey, encoding, env = 'dev'}) {
    ...
    // prepend multibase base58 (0x7a / z) encoding
    cryptonymDid += 'z';
    if(encoding === 'ed25519') {
      // ed25519 cryptonyms are multiformat encoded values, specifically:
      // (multicodec 0x30 + ed25519-pub 0xed)
