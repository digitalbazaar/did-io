...
class VeresOneDidDoc {
   ...
   * Only called when generating a new DID Doc (with new keys, etc).
   *
   * @param env
   * @param passphrase
   *
  ...
  async importKeys(data, options = {}) {
    for(let keyData of Object.values(data)) {
      let key = await LDKeyPair.from(keyData, {injector: this.injector, ...options});
