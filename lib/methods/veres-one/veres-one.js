...
class VeresOne {
  constructor(options = {}) {
    ...
    this.keyStore = options.keyStore || storage.keyStore({ledger: this.ledger});
    this.metaStore = options.metaStore || storage.metaStore({ledger: this.ledger});
    this.didStore = options.didStore || storage.didStore({ledger: this.ledger});
  }
  ...
  async generate({didType = 'nym', keyType = Constants.DEFAULT_KEY_TYPE,
    ...
    await this.keyStore.put(didDoc.id, await didDoc.exportKeys());
    return didDoc;
  }
